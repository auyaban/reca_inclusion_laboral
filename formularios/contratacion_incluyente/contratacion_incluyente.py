...
def sync_usuarios_reca(cache=None, env_path=".env"):
    ...
    empresa_nit = str(section_1.get("nit_empresa") or SECTION_1_CACHE.get("nit_empresa") or "").strip()
    empresa_nombre = str(section_1.get("nombre_empresa") or SECTION_1_CACHE.get("nombre_empresa") or "").strip()
    rows = []
    for entry in data:
...
def export_to_excel(clear_cache=True, cache=None):
    ...
    vinculados = _get_section_2_entries(cache_data.get("section_2", []))
    ...
    writes.extend(_build_section_6_writes(cache_data.get("section_6", {}), num_vinculados=num_vinculados))
    writes.extend(_build_section_7_writes(cache_data.get("section_7", []), num_vinculados=num_vinculados))
    row_insertions = []
    ...
    section_7_raw = cache_data.get("section_7") or []
    asistentes = [
